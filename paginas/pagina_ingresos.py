...
def datos_importantes(mon):   
    ...
    col1.metric(label=f"Total ingresos ({filtro_sueldo.ano.iloc[0]}-{filtro_sueldo.ano.iloc[-1]} )" ,value=f"{st.session_state['datos_imp_sueltos_historicos_totales']:,.0f}")
        
    col2.metric(label=f"Ultimo sueldo ({filtro_sueldo.mes.iloc[-1]}/{filtro_sueldo.ano.iloc[-1]}) " ,value=f"{st.session_state['datos_imp_ultimo_sueldo']:,.0f}",delta=f"{round(variacion_ultimo_sueldo,1)*100}%")
    
    col3.metric(label=f"Ultimo sueldo anual ( {st.session_state['ultimo_ano']})" ,value=f"{st.session_state['datos_imp_sueldos_totales_ultimo_ano']:,.0f}",delta=f"{round(variacion_sueldo_ultimo_ano,1)*100}%")
...
def graf_mejor_mes():
    ...
    fig = px.bar(filtro, x='mes', y=st.session_state['moneda'], color=st.session_state['moneda'],
                 labels={st.session_state['moneda']: 'Ingresos mensuales', 'mes': 'Mes'})
